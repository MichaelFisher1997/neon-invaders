...
function UICosmetics.touchpressed(id, x, y, dx, dy, pressure)
  local vw, vh = love.graphics.getWidth(), love.graphics.getHeight()
  local layout = getLayout(vw, vh)
  
  -- Convert to virtual coordinates
  local lx, ly = x / vw * layout.virtualWidth, y / vh * layout.virtualHeight
  ...
  -- Check tab buttons
  local colorTab = {x = vw/2 - 150, y = 80, w = 120, h = 40}
  local shapeTab = {x = vw/2 + 30, y = 80, w = 120, h = 40}
  
  if lx >= colorTab.x and lx <= colorTab.x + colorTab.w and
     ly >= colorTab.y and ly <= colorTab.y + colorTab.h then
    state.tab = "colors"
    audio.play('ui_click')
    return
  end
  
  if lx >= shapeTab.x and lx <= shapeTab.x + shapeTab.w and
     ly >= shapeTab.y and ly <= shapeTab.y + shapeTab.h then
    state.tab = "shapes"
    audio.play('ui_click')
    return
  end
  ...
  -- Check item clicks
  local itemsY = layout.items.y
  local itemHeight = layout.items.itemHeight
  local visibleCount = layout.items.visibleCount
  
  if state.tab == "colors" then
    local colors = getColors()
    for i = 1, visibleCount do
      local itemIndex = state.colorScroll + i
      if itemIndex <= #colors then
        local itemY = itemsY + (i - 1) * itemHeight
        if lx >= layout.items.x and lx <= layout.items.x + layout.items.width and
           ly >= itemY and ly <= itemY + itemHeight then
          state.colorSelection = itemIndex
          local color = colors[itemIndex]
          if Cosmetics.isColorUnlocked(color.id) then
            Cosmetics.selectColor(color.id)
            showMessage("Color equipped!")
          else
    ...
    local shapes = getShapes()
    for i = 1, visibleCount do
      local itemIndex = state.shapeScroll + i
      if itemIndex <= #shapes then
        local itemY = itemsY + (i - 1) * itemHeight
        if lx >= layout.items.x and lx <= layout.items.x + layout.items.width and
           ly >= itemY and ly <= itemY + itemHeight then
          state.shapeSelection = itemIndex
          local shape = shapes[itemIndex]
          if Cosmetics.isShapeUnlocked(shape.id) then
            Cosmetics.selectShape(shape.id)
            showMessage("Shape equipped!")
          else
...
function UICosmetics.touchmoved(id, x, y, dx, dy, pressure)
  if touchStartY then
    local dragDistance = math.abs(y - touchStartY)
    if dragDistance > 10 then -- Threshold for drag detection
      isDragging = true
      local layout = getLayout(love.graphics.getWidth(), love.graphics.getHeight())
      local scrollDelta = -dy / layout.items.itemHeight
      currentScroll = currentScroll + scrollDelta
      
      if state.tab == "colors" then
        local colors = getColors()
        state.colorScroll = math.max(0, math.min(#colors - layout.items.visibleCount, currentScroll))
      else
        local shapes = getShapes()
        state.shapeScroll = math.max(0, math.min(#shapes - layout.items.visibleCount, currentScroll))
      end
    end
  end
end

function UICosmetics.touchreleased(id, x, y, dx, dy, pressure)
  if isDragging and touchStartY and touchStartTime then
    -- Calculate velocity for momentum scrolling
    local touchDuration = love.timer.getTime() - touchStartTime
    if touchDuration > 0 then
      local velocity = (touchStartY - y) / touchDuration
      scrollVelocity = velocity / 1000 -- Normalize velocity
    end
  end
